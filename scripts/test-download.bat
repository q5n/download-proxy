...
@rem WebClient version faster
@rem powershell -NoProfile -c "&{param($u)$t=[DateTimeOffset]::UtcNow.ToUnixTimeSeconds();$c=[Text.Encoding]::UTF8;$L='%DlProxy%?url='+[Net.WebUtility]::UrlEncode($u)+'&time='+$t+'&sign='+([BitConverter]::ToString([Security.Cryptography.HMACSHA256]::new($c.GetBytes('%DlSecret%')).ComputeHash($c.GetBytes('url='+$u+'&time='+$t))) -replace '-','').ToLower();[Net.WebClient]::new().DownloadFile($L,([uri]$u).Segments[-1])} '%DlTarget%'"
